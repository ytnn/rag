...
def hybrid_search(query, top_k=TOP_K):
    ...
    merged = {r["idx"]: r for r in faiss_results}
    for r in bm25_results:
        if r["idx"] in merged:
            merged[r["idx"]]["score"] += r["score"]
            merged[r["idx"]]["source"] = "hybrid"
        else:
            merged[r["idx"]] = r
...
"""
    ...
    prompt = f"""Ты помощник, который сначала размышляет шаг за шагом, а потом дает ответ на вопрос. Используй только предоставленный контекст".
...
def rag_query_openai(user_query, top_k=TOP_K, distance_threshold=12):
          
        search_result = hybrid_search(user_query, top_k=top_k)

        filtered = [r for r in search_result if r.get("distance", 0) <= distance_threshold or r.get("source") != "faiss"]
        if not filtered:
            return "Я не знаю."
